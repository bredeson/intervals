...
BUILD_TARGETS = $(patsubst src/%,$(LIBRARY_DIR)/%,$(SOURCE_FILES))

INSTALL_TARGETS = $(patsubst src/%,$(INSTALL_PATH)/%,$(SOURCE_FILES))
...
test: $(BUILD_TARGETS)
	PYTHONPATH="$(PWD)/$(LIBRARY_DIR)" $(PYTHON) -m unittest discover test -v
...
$(INSTALL_PATH)/$(PACKAGE)/%.py: $(LIBRARY_DIR)/$(PACKAGE)/%.py
	$(INSTALL_REG) $< $@
